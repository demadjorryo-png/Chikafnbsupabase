 ...
 *
 * - generateChallenges - A function that creates sales challenges based on a budget.
...
const ChallengeGeneratorInputSchema = z.object({
  budget: z.number().describe('The total monthly budget available for challenge rewards.'),
});
...
const prompt = ai.definePrompt({
  ...
  output: { schema: ChallengeGeneratorOutputSchema },
  prompt: `You are Chika AI, an expert in designing employee incentive programs for a vape store called "Bekupon Vape Store".

Your task is to generate a set of 3-4 tiered sales challenges for employees based on a total monthly reward budget. The challenges should be based on achieving a certain total sales revenue (omset) in Indonesian Rupiah (Rp).

The tiers should be creative and motivating (e.g., "Vape Rookie", "Cloud Chaser", "Master Puffer").
The targets should be realistic but challenging for a vape store employee, starting from a reasonable base and increasing for each tier.
...
Total Monthly Budget: Rp {{budget}}
